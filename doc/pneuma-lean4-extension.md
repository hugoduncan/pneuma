...
11. [Human-Readable Proofs](#11-human-readable-proofs)
...
## 11. Human-Readable Proofs

Pneuma's proofs aren't being written by mathematicians. They're being written and read by Clojure developers who need to understand *why* an architectural invariant holds, not just *that* it holds. A proof that says `by decide` tells the kernel everything and the developer nothing. Readability is not optional — it's the difference between a proof that builds understanding and one that builds an opaque wall of trust.

Lean 4 has the machinery to present proofs in human-readable form, but it doesn't happen by default. It requires deliberate choices at three levels: inside the `.lean` files, in an external blueprint document, and in the dependency graph that ties everything together.

### 11.1. Tier 1 — Docstrings in the Lean files

Every generated theorem gets a `/-- ... -/` documentation comment that states what the theorem means in plain English. These are generated by `->lean` alongside the type definitions and proof scaffolding. In VS Code with the Lean extension, hovering over a theorem name shows the docstring — so a developer navigating the proof files gets an English explanation without switching context.

```lean
/-- The session lifecycle chart never reaches a configuration where
    the conversation is in tool-executing while a tool-error has been
    raised. This holds because both states are in the same non-parallel
    region, so only one can be active at a time. -/
theorem toolExec_toolError_exclusive :
    ∀ c : Config, reachable c →
      ¬(c.conv = .toolExecuting ∧ c.conv = .toolError) := by
  intro c ⟨events, hc⟩
  intro ⟨h1, h2⟩
  rw [h1] at h2
...
Docstrings are the minimum viable readability. They cost nothing to generate (the `->lean` projection already knows what the theorem means — it was derived from a named Clojure formalism), and they make the `.lean` files navigable by someone who doesn't read Lean.

The `->lean` projection also generates docstrings for each type definition, linking back to the Pneuma formalism it was derived from:

```lean
/-- Conversation states for the session lifecycle chart.
    Derived from Pneuma statechart :SessionLifecycle.
    See: agent.formalisms/session-chart -/
inductive ConvState where
  | idle
  | generating
  | awaitingApproval
...
### 11.2. Tier 2 — Lean Blueprint integration

The Blueprint tool, created by Patrick Massot and used in major formalization projects including the Liquid Tensor Experiment and Terence Tao's PFR conjecture proof, provides a dual-representation system: a human-readable LaTeX document linked to a machine-checked Lean formalization. Each theorem exists in both forms. The LaTeX version is what a developer reads; the Lean version is what the kernel checks. Special macros connect the two.

The `->lean` projection emits not just `.lean` files but also a LaTeX blueprint. The resulting file structure:

```
proofs/
├── lakefile.lean
├── Pneuma/
│   ├── Chart.lean                 ← generated: types + step function
│   ├── Effects.lean               ← generated: effect signature types
│   ├── Morphisms.lean             ← generated: boundary propositions
│   └── Cycles.lean                ← generated: composition theorems
├── Proofs/
│   ├── ChartSafety.lean           ← human-written proofs
│   ├── Replay.lean
│   ├── Composition.lean
│   └── CycleClosure.lean
└── blueprint/
    ├── src/
    │   └── content.tex            ← human-readable proofs (generated + annotated)
    └── web/
        ├── index.html             ← browsable proof document
        └── dep_graph.html         ← dependency graph with proof status
```

The LaTeX blueprint for a chart safety proof reads as normal mathematical prose:

```latex
\begin{theorem}[Chart safety]\label{thm:chart_safety}
\lean{chart_safety}
\leanok
\uses{def:conv_state, def:config, def:step, def:reachable}
No reachable configuration of the session lifecycle chart contains
both \texttt{:tool-executing} and \texttt{:tool-error} as active
conversation states.
\end{theorem}

\begin{proof}
\leanok
\uses{thm:conv_state_noconfusion}
Both states belong to the \texttt{conversation} region, which is
non-parallel. In any configuration, exactly one conversation state
is active. If we assume both \texttt{toolExecuting} and
\texttt{toolError} are active, we derive a contradiction from
the injectivity of the \texttt{ConvState} constructors.
\end{proof}
```

The `\lean{chart_safety}` macro links this to the Lean declaration. The `\leanok` macro flags that the proof is formalized. The `\uses{...}` macro declares dependencies, which Blueprint uses to generate the dependency graph.

The generated website is browsable: clicking a theorem node in the dependency graph shows the human-readable statement and proof, with a link to the Lean source. Color coding shows status: green for fully formalized, blue for statement formalized but proof pending, white for neither.

### 11.3. Tier 3 — Structured proof style in Lean

Where possible, the proofs themselves — not just the docstrings and blueprints — should be readable. Lean 4 provides constructs for this:

**The `calc` tactic** presents proofs as chains of inequalities or equalities, resembling a structured mathematical argument:

```lean
theorem generation_stays_in_region :
    ∀ events : List Event,
      let c := events.foldl step initialConfig
      c.conv ∈ conversationStates := by
  intro events
  induction events with
  | nil => simp [initialConfig, conversationStates]
  | cons e es ih =>
    simp [List.foldl]
    calc (es.foldl step initialConfig |> fun c => step c e).conv
        _ ∈ conversationStates := by
          have h := ih
          cases e <;> simp [step] <;> assumption
```

**The `have` tactic** introduces intermediate assertions, giving names to sub-goals that explain the reasoning:

```lean
theorem cycle_closes_for_generation :
    ∀ t ∈ raisingTransitions,
      let callback := callbackOf (effectOf (handlerOf t.raise))
      chart.accepts (t.target) callback := by
  intro t ht
  -- The transition raises :start-generation
  have h_raise : t.raise = .startGeneration := by ...
  -- The handler for :start-generation emits ai/generate
  have h_effect : effectOf (handlerOf t.raise) = .aiGenerate := by ...
  -- ai/generate's callback is :generation-complete
  have h_callback : callbackOf .aiGenerate = .generationComplete := by ...
  -- The target state after the transition is :generating
  have h_target : t.target = .generating := by ...
  -- :generating accepts :generation-complete
  show chart.accepts .generating .generationComplete
  decide
```

This reads as a structured argument: "The transition raises `:start-generation`. The handler emits `ai/generate`. The callback is `:generation-complete`. The target state is `:generating`. And `:generating` accepts `:generation-complete`." Each `have` step is a claim with a name and a justification. The `decide` at the end says "and this is verified by exhaustive check."

**The `suffices` tactic** works backwards from the goal, which is natural for many architectural arguments:

```lean
theorem no_unauthorized_dispatch (ext : Extension) (e : Event) :
    dispatches ext e → e ∈ ext.caps.dispatch := by
  -- It suffices to show the interceptor blocks unauthorized events
  suffices h : ∀ e, e ∉ ext.caps.dispatch → ¬dispatches ext e by
    contrapose!; exact h e
  intro e h_unauth
  -- The permission interceptor checks capability membership
  exact interceptor_blocks_unauthorized ext e h_unauth
```

For the finite-state properties common in Pneuma (chart safety, morphism composition), many proofs close with `by decide` — the machine checks every case. This is opaque but trustworthy. The human-readable content lives in the Blueprint document, not in the tactic proof. The `.lean` file's job is to convince the kernel; the blueprint's job is to convince the developer.

### 11.4. The dependency graph as proof map

Blueprint generates a dependency graph from the `\uses` annotations. For Pneuma, this graph has a specific structure that mirrors the three-layer formalism architecture:

- **Leaf nodes** are object-level definitions and theorems: state types, step functions, effect types, chart safety invariants.
- **Middle nodes** are morphism-level theorems: existential reference completeness, structural match validity, containment subset proofs.
- **Root nodes** are path-level theorems: cycle closure, precondition chaining, convergence.

Each node is color-coded: green (fully proved), blue (statement formalized, proof pending), white (not yet formalized), with borders indicating whether prerequisites are met. The graph is browsable — clicking a node shows the human-readable theorem statement and proof from the blueprint, plus a link to the Lean source.

This dependency graph is the artifact you show in a design review. It answers "which architectural invariants have we proved?" at a glance, and for any unproved node, it shows exactly which prerequisites are blocking it. It is the proof-level analog of Pneuma's gap report: the gap report shows where the *implementation* diverges from the *specification*; the Blueprint dependency graph shows where the *proofs* of the *specification's properties* are complete or pending.

### 11.5. LeanAide for bidirectional translation

LeanAide, a tool for AI-assisted autoformalization, provides bidirectional translation between natural language and Lean 4. It can generate documentation strings from Lean declarations and translate informal theorem statements into formal ones.

For Pneuma, LeanAide could serve two roles. First, enriching the generated `.lean` files with natural-language docstrings that are more contextual than what `->lean` produces from the Clojure data alone — incorporating information about *why* the theorem matters, not just *what* it states. Second, providing a REPL-like interface where a developer can state a property in English ("the callback event is always accepted by the chart") and LeanAide translates it into a Lean theorem statement that can be proved.

This is speculative — LeanAide's quality for domain-specific properties (as opposed to textbook mathematics) is unproven. But the architecture accommodates it: since the type definitions are generated by `->lean`, LeanAide has a well-typed context to work within, which should improve its translation quality compared to working from scratch.

### 11.6. The readability contract

Pneuma commits to the following readability guarantees for its Lean integration:

- Every generated type definition has a docstring linking it to the Clojure formalism.
- Every generated theorem statement has a docstring in plain English.
- Every proof target is accompanied by a Blueprint LaTeX entry with a human-readable proof sketch.
- The Blueprint dependency graph is generated and publishable as a static website.
- Proofs of finite-state properties use `decide` (opaque but trivially trustworthy). Proofs of structural properties use `have` steps with named intermediate goals (readable but longer).

The human-readable layer is not optional. A proof that nobody can read is a proof that nobody will maintain, and an unmaintained proof will eventually be deleted. The Blueprint is the mechanism that prevents this — it gives the proofs a life outside the `.lean` files, as a document that can be read, discussed, and reviewed by people who don't know Lean.

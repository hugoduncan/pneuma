...
Seven new emission modules that prove properties beyond dispatch
validation. All proofs use `decide` on finite structures — no
hand-written Lean needed. `bin/self-proof` emits all layers per spec
with namespace wrapping. `Proofs.lean` stays empty.

**Phase 14a — Independent verification modules:**

- [x] `pneuma.lean.morphism-algebra` — composition, identity, associativity over the morphism graph
- [x] `pneuma.lean.circuit` — verify Johnson's algorithm output for concrete graphs
- [x] `pneuma.lean.gap-completeness` — every morphism/formalism/path was checked in the gap report
- [x] `pneuma.lean.monitor-schema` — per-formalism gap-kind and status enumeration exhaustiveness
- [x] `pneuma.lean.ref-exhaustive` — per-formalism ref-kind extraction covers all used ref-kinds

**Phase 14b — Composition and path verification:**

- [x] `pneuma.lean.composition-transitivity` — bridge functions between adjacent morphism endpoints, totality proofs
- [x] `pneuma.lean.path-semantic` — full cycle semantic composition, ref universe preservation

**Phase 14c — Integration and emission:**

- [x] Update `pneuma.lean.core` — 7 new public API functions, `emit-lean-all` returns 7 additional keys
- [x] Update `bin/self-proof` — emit all verification layers per spec with namespace wrapping
- [x] 248 Lean files (31 system + 217 verification) build with `lake build`
- [x] Document bb tasks in AGENTS.md
...
pneuma.lean.protocol (no deps)
  ...
  ├── pneuma.lean.morphism-algebra ── (lean.doc + path.graph)
  ├── pneuma.lean.circuit ── (lean.doc + path.graph)
  ├── pneuma.lean.gap-completeness ── (lean.doc)
  ├── pneuma.lean.monitor-schema ── (lean.doc + protocol)
  ├── pneuma.lean.ref-exhaustive ── (lean.doc)
  ├── pneuma.lean.composition-transitivity ── (lean.doc + protocol)
  ├── pneuma.lean.path-semantic ── (lean.doc + protocol)
  ├── pneuma.lean.blueprint ── (lean.core)
  └── pneuma.lean.core ── (all lean.* namespaces + gap.core + path.core)

proofs/                         (Lean 4 — generated by ->lean, not Clojure)
  ├── Pneuma/{Spec}.lean        (system-level: dispatch validation per spec)
  ├── Pneuma/{Spec}Algebra.lean (morphism graph algebra per spec)
  ├── Pneuma/{Spec}Circuits.lean (Johnson's output verification per spec)
  ├── Pneuma/{Spec}GapComplete.lean (gap report completeness per spec)
  ├── Pneuma/{Spec}MonitorSchema.lean (monitor-schema consistency per spec)
  ├── Pneuma/{Spec}RefExhaustive.lean (ref-extraction exhaustiveness per spec)
  ├── Pneuma/{Spec}CompTransitivity.lean (composition transitivity per spec)
  ├── Pneuma/{Spec}PathSemantic.lean (path semantic composition per spec)
  ├── Proofs.lean               (hand-written proofs — currently empty)

...
- [x] `pneuma.protocol-spec` — formalism spec of pneuma.protocol (in spec/)
  - EffectSignature instance for the eight protocol methods
...
Adds the `->lean` projection to `IProjectable` and `IConnection`,
emitting Lean 4 source code for kernel-verified architectural
invariants. Lean proves properties of the specification (universal);
Pneuma tests conformance of the implementation (sampled). See
...
- [ ] `->lean` projection on `IProjectable` and `IConnection` protocols
- [ ] Step 1: Statechart emission — inductive types, step function, chart safety theorems
- [ ] Step 2: Effect signature emission — inductive Effect type and field structures
- [ ] Step 3: Mealy handler emission — handler contracts, replay determinism proof target
- [ ] Step 4: Morphism emission — boundary propositions, composition theorems
- [ ] Step 5: Cycle emission — cycle-level theorems (event-effect-callback closure)
- [ ] Step 6: Bisimulation emission — bisimulation checker for statechart pairs

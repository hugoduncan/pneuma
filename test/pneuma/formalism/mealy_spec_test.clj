(ns pneuma.formalism.mealy-spec-test
    (:require [clojure.test :refer [deftest testing is]]
              [pneuma.protocol :as p]
              [pneuma.formalism.capability :as cap]
              [pneuma.formalism.mealy-spec :as spec]
              [pneuma.gap.core :as gap]))

;; Specification test: pneuma checks its own mealy namespace.

(deftest mealy-formalisms-test
  ;; The specification instances are well-formed.
         (testing "mealy spec formalisms"
                  (testing "api operations has one operation"
                           (is (= 1 (count (p/extract-refs spec/mealy-api-operations
                                                           :operation-ids)))))

                  (testing "ref operations has five ref-kinds"
                           (is (= 5 (count (p/extract-refs spec/mealy-ref-operations
                                                           :operation-ids)))))

                  (testing "api caps match api operations"
                           (is (= (p/extract-refs spec/mealy-api-caps :dispatch-refs)
                                  (p/extract-refs spec/mealy-api-operations :operation-ids))))

                  (testing "ref-kind caps match ref operations"
                           (is (= (p/extract-refs spec/mealy-ref-kind-caps :dispatch-refs)
                                  (p/extract-refs spec/mealy-ref-operations :operation-ids))))

                  (testing "type schema covers all output types"
                           (let [api-outputs (p/extract-refs spec/mealy-api-operations
                                                             :operation-outputs)
                                 ref-outputs (p/extract-refs spec/mealy-ref-operations
                                                             :operation-outputs)
                                 known-types (p/extract-refs spec/mealy-types :type-ids)]
                                (is (every? known-types api-outputs))
                                (is (every? known-types ref-outputs))))))

(deftest mealy-gap-report-test
         (let [report (spec/mealy-gap-report)]
              (testing "mealy gap report"
         ...
         (testing "gap report detects simulated missing ref-kind"
                  (let [bad-caps (cap/capability-set
                        ...
                        report (gap/gap-report
                                {:formalisms
                                 (assoc spec/mealy-formalisms
                                        :capability-set/mealy-ref-kinds bad-caps)
                                 :registry spec/mealy-registry})]
